...
pacinit()
{
	...
	Pacman.adjusttime = 0;
	paccomp();
	eladd(Pacman.time+STARTDELAY,pacmove, NULL, 0);
}
/*
...
pacmove()
{
	 ...
	 *  having already been computed, for efficiency's sake.
	 */
	Pacman.mrow = SCtoMZ(Pacman.row+2);
	Pacman.mcol = SCtoMZ(Pacman.col+2);
	/*
	 * Check for walls going off and gobbling a dot
	 */
	Pacman.moves++;
	if ( !Wallsoff && Pacman.moves>(WALLSDISAPPEAR-50*max(Wave,15)) ) {
		compwalls();
		Wallsoff = TRUE;
	}
	if ( hasdot() ) {
	...
	eladd(Pacman.time+Pacman.adjusttime,pacmove, NULL, 0);
}
paccomp()
{
 ...
 *
 * Parameters are set for 9 secs - 0.5 secs per wave, min 2 sec
 */
int
pacpower(junk,type)
char *junk;
...
/*		soundpill():	*/
		...
		for ( mptr=Monster ; mptr<&Monster[MAXMONSTER] ; mptr++ ) {
			...
			if ( mptr->state == DANGEROUS || mptr->state == EDIBLE ) {
...
register monster *mptr;
{
	return (iabs(mptr->row-Pacman.row)+iabs(mptr->col-Pacman.col)<3 );
}
/*
